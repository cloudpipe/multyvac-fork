...
logger = logging.getLogger('multyvac.cloudpickle')
...
try:
    ...
    logger.warning('Exception raised on importing ctypes. Likely python bug, '
...
class CloudPickler(pickle.Pickler):
    ...
    def save_function(self, obj, name=None, pack=struct.pack): 
        ...
        if modname == 'cloud.shell':
            ...
            if django_settings:
                ...
                if django_mod:
                    logger.debug('Transporting django settings %s during save of %s', django_mod, name)
                    self.savedDjangoEnv = True
    ...
    def save_function_tuple(self, func, forced_imports):
        ...
        if forced_imports:
            ...
            forced_names = [m.__name__ for m in forced_imports if not m.__name__.startswith('multyvac')]         
            save((forced_names,))
